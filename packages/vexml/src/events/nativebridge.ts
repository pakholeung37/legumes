...
export class NativeBridge<V extends string> {
  ...
  activate(vexmlEventName: V) {
    util.assert(
      !this.isActivated(vexmlEventName),
      `vexml event is already active: ${vexmlEventName}`,
    )
    ...
    for (const native of Object.entries(mapping.native)) {
      ...
      if (!this.nativeEventTopic.hasSubscribers(nativeEventName)) {
        const srcElement =
          mapping.src === 'scroll'
            ? this.root.getScrollContainer()
            : this.root.getOverlay().getElement()
      ...
      const handle = this.nativeEventTopic.subscribe(
        nativeEventName,
        nativeEventListener,
      )
  ...
  deactivate(vexmlEventName: V) {
    util.assert(
      this.isActivated(vexmlEventName),
      `vexml event is already inactive: ${vexmlEventName}`,
    )
  ...
  private publishNativeEvent = (event: Event) => {
    this.nativeEventTopic.publish(
      event.type as keyof HTMLElementEventMap,
      event,
    )
    return false
  }
