...
export class OctaveShift {
  ...
  private renderVexflowTextBrackets(
    ...
    if (renders.length < 2) {
      this.log.warn(
        'cannot render octave shift with less than 2 notes, skipping',
        {
    ...
    for (const systemIndex of systemIndexes) {
      const systemNoteRenders = renders.filter(
        (n) => n.key.systemIndex === systemIndex,
      )
      if (systemNoteRenders.length > 1) {
        const vexflowTextBracket =
          this.renderVexflowTextBracket(systemNoteRenders)
