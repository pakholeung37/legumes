...
export class Stave {
  ...
  render(): StaveRender {
    ...
    const vexflowStave = isTabStave
      ? new vexflow.TabStave(this.position.x, this.position.y, 0, {
          numLines: staveLineCount,
        })
      : new vexflow.Stave(this.position.x, this.position.y, 0, {
          numLines: staveLineCount,
        })

    const { voiceRenders, vexflowMultiMeasureRest } =
      this.renderVoicesOrVexflowMultiMeasureRest(vexflowStave)
  ...
  private renderVoicesOrVexflowMultiMeasureRest(vexflowStave: vexflow.Stave): {
    ...
    if (multiRestCount > 1) {
      ...
      vexflowMultiMeasureRest = new vexflow.MultiMeasureRest(multiRestCount, {
        numberOfMeasures: 1,
      })
    } else {
      for (let voiceIndex = 0; voiceIndex < voiceCount; voiceIndex++) {
        const key = { ...this.key, voiceIndex }
        const voiceRender = new Voice(
          this.config,
          this.log,
          this.document,
          key,
  ...
  private renderStartClef(vexflowStave: vexflow.Stave): ClefRender | null {
    const isClefEnabled = this.document.isTabStave(this.key)
      ? this.config.SHOW_TAB_CLEF
      : true
    if (!isClefEnabled) {
      return null
    }
  ...
  private renderTime(vexflowStave: vexflow.Stave): TimeRender | null {
    const isTabStave = this.document.isTabStave(this.key)
    const isTimeEnabled = isTabStave
    ...
    const previousTime = this.document.getPreviouslyPlayedStave(this.key)
      ?.signature.time
  ...
  private renderBarlines(vexflowStave: vexflow.Stave): void {
    ...
    const startVexflowBarlineType = this.toVexflowBarlineType(
      this.document.getMeasure(this.key).startBarlineStyle,
    )
    ...
    const endVexflowBarlineType = this.toVexflowBarlineType(
      this.document.getMeasure(this.key).endBarlineStyle,
    )
  ...
  private toVexflowBarlineType(
    barlineStyle: data.BarlineStyle | null,
  ): vexflow.BarlineType {
    switch (barlineStyle) {
      case 'single':
  ...
  private adjustStems(voiceRenders: VoiceRender[]): void {
    const voices = voiceRenders.filter((voice) =>
      voice.entryRenders.some((entry) => entry.type === 'note'),
    )
    if (voices.length <= 1) {
      return
    }
    ...
    for (const entry of top.entryRenders) {
      if (
        entry.type == 'note' &&
        entry.stemDirection === 'auto' &&
        entry.vexflowNote instanceof vexflow.StaveNote
      ) {
  ...
  private renderKeySignature(vexflowStave: vexflow.Stave): KeyRender | null {
    ...
    const previousKey = this.document.getPreviouslyPlayedStave(this.key)
      ?.signature.key
  ...
  private renderMetronome(vexflowStave: vexflow.Stave): void {
    ...
    const currentMetronome = this.document.getFragment(this.key).signature
      .metronome
    const previousMetronome = this.document.getPreviousFragment(this.key)
      ?.signature.metronome
    ...
    const hasMetronome =
      currentMetronome.displayBpm ||
      currentMetronome.dots ||
      currentMetronome.dots2 ||
      currentMetronome.duration

    if (
      hasMetronome &&
      isFirstPart &&
      isFirstStave &&
      (isAbsolutelyFirst || didMetronomeChange)
    ) {
      vexflowStave.setTempo(
        { ...currentMetronome, bpm: currentMetronome.displayBpm },
        -METRONOME_TOP_PADDING,
      )
    }
  }
