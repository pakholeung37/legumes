...
export class Score {
  ...
  render(): ScoreRender {
    ...
    const width =
      this.width ?? util.max(systemRenders.map((system) => system.rect.w))
  ...
  private renderOctaveShifts(registry: RenderRegistry): OctaveShiftRender[] {
    ...
    for (
      let octaveShiftIndex = 0;
      octaveShiftIndex < octaveShifts.length;
      octaveShiftIndex++
    ) {
      const key: OctaveShiftKey = { octaveShiftIndex }

      const noteRenderCount =
        registry.get(octaveShifts[octaveShiftIndex].id)?.length ?? 0

      if (noteRenderCount >= 1) {
        const octaveShiftRender = new OctaveShift(
          this.config,
          this.log,
          this.document,
          key,
  ...
  private renderVibratos(registry: RenderRegistry): VibratoRender[] {
    ...
    for (let vibratoIndex = 0; vibratoIndex < vibratos.length; vibratoIndex++) {
      ...
      const noteRenderCount =
        registry.get(vibratos[vibratoIndex].id)?.length ?? 0

      if (noteRenderCount >= 1) {
        const vibratoRender = new Vibrato(
          this.config,
          this.log,
          this.document,
          key,
