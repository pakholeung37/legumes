...
export class Document {
  ...
  getOctaveShiftKey(id: string): OctaveShiftKey {
    ...
    util.assertDefined(octaveShift)
    return {
      octaveShiftIndex: this.data.score.octaveShifts.indexOf(octaveShift),
    }
  }
      ...
      for (let partIndex = 0; partIndex < partCount; partIndex++) {
        ...
        for (let staveIndex = 0; staveIndex < staveCount; staveIndex++) {
          ...
          if (measureMultiRestCount === -1) {
            ...
            measureMultiRestCount = Math.min(
              measureMultiRestCount,
              this.getStaveMultiRestCount(staveKey),
            )
          }
        }
      }
  ...
  getNonMusicalFragment(key: FragmentKey): data.NonMusicalFragment {
    ...
    util.assert(
      fragment.kind === 'nonmusical',
      'expected a non-musical fragment',
    )
    return fragment
  }
  ...
  getPreviouslyPlayedStave(key: StaveKey): data.Stave | null {
    return (
      this.getPreviousFragment(key)
        ?.parts.at(key.partIndex)
        ?.staves.at(key.staveIndex) ?? null
    )
  }
  ...
  getArticulations(key: VoiceEntryKey): data.Articulation[] {
    ...
    util.assert(
      ...
      'expected entry to be a note or chord',
    )
