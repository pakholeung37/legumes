...
export class Wedge {
  ...
  private renderVexflowStaveHairpins(
    renders: Array<NoteRender | RestRender>,
  ): vexflow.StaveHairpin[] {
    if (renders.length < 2) {
      this.log.warn('cannot render wedge with less than 2 notes, skipping', {
        wedgeIndex: this.key.wedgeIndex,
      })
      return []
    }
    ...
    for (const systemIndex of systemIndexes) {
      ...
      if (systemScopedRenders.length > 1) {
        const vexflowStaveHairpin =
          this.renderSingleStaveHairpin(systemScopedRenders)
  ...
  private renderSingleStaveHairpin(
    renders: Array<NoteRender | RestRender>,
  ): vexflow.StaveHairpin {
    const wedge = this.document.getWedge(this.key)
    ...
    return new vexflow.StaveHairpin(
      { firstNote, lastNote },
      vexflowStaveHairpinType,
    ).setPosition(vexflowModifierPosition)
  }
}
