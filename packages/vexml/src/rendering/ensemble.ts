...
export class Ensemble {
  ...
  format(
    ...
    { x, width, paddingLeft, paddingRight, cache }: EnsembleFormatParams,
  ): void {
    util.assert(
      fragmentRender.rectSrc === 'none',
      'expected fragment render to be unformatted',
    )

    const vexflowFormatter = new vexflow.Formatter()

    // Explode out the components of the ensemble.
    const staveRenders = fragmentRender.partRenders.flatMap(
      (p) => p.staveRenders,
    )
    const vexflowStaves = staveRenders.map((s) => s.vexflowStave)
    const voiceRenders = staveRenders.flatMap((s) => s.voiceRenders)
    const vexflowVoices = voiceRenders
    ...
    const maxNoteStartX = Math.max(
      ...vexflowStaves.map((v) => v.getNoteStartX()),
    )
    const noteStartOffsetXes = vexflowStaves.map(
      (v) => maxNoteStartX - v.getNoteStartX(),
    )
  ...
  private draw(
    fragmentRender: FragmentRender,
    {
      ...
      paddingLeft,
      paddingRight,
    }: { x: number; paddingLeft: number; paddingRight: number },
  ): void {
      ...
      for (const staveRender of partRender.staveRenders) {
        for (const voiceRender of staveRender.voiceRenders) {
          for (const entryRender of voiceRender.entryRenders) {
            entryRender.rect = this.hackEntryRenderRect(
              entryRender,
              staveRender,
            )
          }
          voiceRender.rect = Rect.merge(
            voiceRender.entryRenders.map((e) => e.rect),
          )
        }
        staveRender.rect = this.overrideVexflowStaveRect(
          staveRender,
          x,
          paddingLeft,
          paddingRight,
        )
        staveRender.intrinsicRect =
          this.calculateStaveIntrinsicRect(staveRender)
    ...
    fragmentRender.rect = Rect.merge(
      fragmentRender.partRenders.map((s) => s.rect),
    )
  ...
  private hydrate(fragmentRender: FragmentRender, cache: FragmentRender): void {
    util.assert(
      cache.rectSrc === 'draw',
      'expected cache fragment to be from draw',
    )
    ...
    for (
      let partIndex = 0;
      partIndex < fragmentRender.partRenders.length;
      partIndex++
    ) {
      ...
      for (
        let staveIndex = 0;
        staveIndex < partRender.staveRenders.length;
        staveIndex++
      ) {
        ...
        for (
          let voiceIndex = 0;
          voiceIndex < staveRender.voiceRenders.length;
          voiceIndex++
        ) {
          ...
          for (
            let entryIndex = 0;
            entryIndex < voiceRender.entryRenders.length;
            entryIndex++
          ) {
  ...
  private getCurveExtraWidth(entryRenders: VoiceEntryRender[]): number {
    const curveCount = util.unique(
      entryRenders.filter((e) => e.type === 'note').flatMap((n) => n.curveIds),
    ).length
    if (curveCount <= 1) {
      return 0
    }
  ...
  private getMinRequiredStaveWidth(
    ...
    return (
      this.config.BASE_VOICE_WIDTH / fragmentCount +
      this.getMinRequiredVoiceWidth(noteStartOffsetXes, staveRenders)
    )
  }
  ...
  private getMinRequiredVoiceWidth(
    ...
    const widths = staveRenders.map((s, index) => {
      const vexflowVoices = s.voiceRenders
        .flatMap((v) => v.vexflowVoices)
        .filter((v) => v.getTickables().length > 0)
      if (vexflowVoices.length === 0) {
        return 0
        ...
        return (
          new vexflow.Formatter()
            ...
            .preCalculateMinTotalWidth(vexflowVoices) +
  ...
  private overrideVexflowTextDynamicsRect(
    vexflowTextDynamics: vexflow.TextDynamics,
    staveRender: StaveRender,
  ): Rect {
  ...
  private maybefixVexflowGraceNoteGroupBoundingBox(
    ...
    const boxes = vexflowGraceNoteGroup
      .getGraceNotes()
      .map((n) => n.getBoundingBox())
    if (boxes.length === 0) {
      return
    }
  ...
  private hackEntryRenderRect(
    ...
    if (entryRender.vexflowNote instanceof vexflow.TextDynamics) {
      return this.overrideVexflowTextDynamicsRect(
        entryRender.vexflowNote,
        staveRender,
      )
    }
    ...
    const sanitizedMods = originalMods.filter((m) =>
      PROBLEMATIC_VEXFLOW_MODIFIERS.every((p) => !(m instanceof p)),
    )
