...
export class Note {
  ...
  render(): NoteRender {
    ...
    util.assert(
      voiceEntry.type === 'note' || voiceEntry.type === 'chord',
      'expected note or chord',
    )
    ...
    if (isTabStave) {
      const tabGraceEntries = this.renderTabGraceComponents(
        vexflowNote,
        voiceEntry,
      )
      ...
      const staveGraceEntries = this.renderStaveGraceComponents(
        vexflowNote,
        voiceEntry,
      )
  ...
  ): vexflow.TabNotePosition[] {
    switch (voiceEntry.type) {
      case 'note':
        return voiceEntry.tabPositions.map((t) =>
          this.toVexflowTabNotePosition(t),
        )
      case 'chord':
        return voiceEntry.notes
          .flatMap((note) => note.tabPositions)
          .map((t) => this.toVexflowTabNotePosition(t))
    }
  }
  ...
  private toVexflowTabNotePosition(
    ...
    const fret = tabPosition.harmonic
      ? `<${tabPosition.fret}>`
      : tabPosition.fret
  ...
  private getVexflowStaveNoteKeys(
    ...
    switch (voiceEntry.type) {
      ...
      case 'chord':
        return voiceEntry.notes.map((note) =>
          this.getVexflowNoteKey(note, octaveShift),
        )
    }
  }
  ...
  private renderVexflowAnnotations(
    ...
    for (const annotation of voiceEntry.annotations) {
      ...
      if (annotation.verticalJustification) {
        vexflowAnnotation.setVerticalJustification(
  ...
  private renderVexflowAccidentals(
    ...
    switch (voiceEntry.type) {
      case 'note':
        vexflowAccidentals.push(
          this.renderVexflowAccidental(voiceEntry.accidental),
        )
        break
      case 'chord':
        vexflowAccidentals.push(
          ...voiceEntry.notes.map((note) =>
            this.renderVexflowAccidental(note.accidental),
          ),
        )
        break
    }
  ...
  private renderVexflowAccidental(
    accidental: data.Accidental | null,
  ): vexflow.Accidental | null {
    if (!accidental) {
      return null
    }
  ...
  private renderTabGraceComponents(
    ...
    for (
      let graceEntryIndex = 0;
      graceEntryIndex < voiceEntry.graceEntries.length;
      graceEntryIndex++
    ) {
      ...
      switch (graceEntry.type) {
        case 'gracenote':
          positions.push(
            ...graceEntry.tabPositions.map((tabPosition) =>
              this.toVexflowTabNotePosition(tabPosition),
            ),
          )
          break
        case 'gracechord':
    ...
    const vexflowGraceNoteGroup = new vexflow.GraceNoteGroup(
      vexflowGraceTabNotes,
    )
  ...
  private renderStaveGraceComponents(
    ...
    const octaveShift =
      this.document.getStave(this.key).signature.clef.octaveShift ?? 0
    ...
    for (
      ...
      switch (graceEntry.type) {
        case 'gracenote':
          vexflowAccidentals.push(
            this.renderVexflowAccidental(graceEntry.accidental),
          )
          break
        case 'gracechord':
          vexflowAccidentals.push(
  ...
  private renderArticulations(vexflowNote: vexflow.Note): ArticulationRender[] {
    ...
    for (
      ...
      articulationIndex < articulations.length;
      articulationIndex++
    ) {
      const key: ArticulationKey = { ...this.key, articulationIndex }

      const articulationRender = new Articulation(
        this.config,
        this.log,
        this.document,
        key,
