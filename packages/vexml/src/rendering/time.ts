...
export class Time {
  ...
  render(): TimeRender {
    ...
    const vexflowTimeSignatures = timeSpecs.map(
      (t) => new vexflow.TimeSignature(t),
    )
    const padding =
      ADDITIONAL_COMPLEX_TIME_SIGNATURE_COMPONENT_WIDTH * (timeSpecs.length - 1)
    const width = vexflowTimeSignatures.reduce(
      (sum, t) => sum + t.getWidth(),
      padding,
    )
  ...
  private toFractions(components: data.Fraction[]): Fraction[] {
    return components.map(
      (component) => new Fraction(component.numerator, component.denominator),
    )
  }
}
