...
export class ScoreContext {
  ...
  private wedgeStatuses = new Map<
    string,
    Map<number, { id: string; count: number; delete: boolean }>
  >()

  private pedals = new Array<data.Pedal>()
  // part ID -> pedal status
  private pedalStatuses = new Map<
    string,
    { id: string; next: data.PedalMarkType; count: number; delete: boolean }
  >()

  private octaveShifts = new Array<data.OctaveShift>()
  // part ID -> stave number -> octave shift status
  private octaveShiftStatuses = new Map<
    string,
    Map<number, { id: string; count: number; delete: boolean }>
  >()
  ...
  getMultiRestCount(partId: string, staveNumber: number | null): number {
    return (
      this.multiRestCounts.get(partId)?.get(null) ??
      this.multiRestCounts.get(partId)?.get(staveNumber) ??
      0
    )
  }

  incrementMultiRestCount(
    partId: string,
    ...
    this.multiRestCounts
      .get(partId)!
      .set(staveNumber, count + this.getMultiRestCount(partId, staveNumber))
  }
  ...
  beginWedge(
    partId: string,
    staveNumber: number,
    wedgeType: data.WedgeType,
    placement: data.WedgePlacement,
  ): string {
    const id = this.nextId()
    ...
    this.wedgeStatuses
      .get(partId)!
      .set(staveNumber, { id, count: 0, delete: false })
...
export class MeasureContext {
  ...
  private accidentals: Record<
    string,
    Record<string, Record<number, data.AccidentalCode>>
  > = {}
  ...
  setActiveAccidental(
    ...
    pitch: string,
    octave: number,
    accidental: data.AccidentalCode,
  ): void {
...
export class FragmentContext {
  constructor(
    private measure: MeasureContext,
  ...
  beginCurve(
    ...
    return this.measure.beginCurve(
      curveNumber,
      placement,
      opening,
      articulation,
...
export class PartContext {
  ...
  beginCurve(
    ...
    return this.fragment.beginCurve(
      curveNumber,
      placement,
      opening,
      articulation,
...
export class VoiceContext {
  ...
  beginTuplet(
    number: number,
    showNumber: boolean,
    placement: data.TupletPlacement,
  ): string {
    const id = this.nextId()
