...
export class Curve {
  ...
  static create(
    config: Config,
    log: Logger,
    musicXML: { notation: musicxml.Notations },
  ): Curve[] {
    return [
      ...Curve.createSlurs(config, log, musicXML),
      ...Curve.createTies(config, log, musicXML),
    ]
  }
  ...
  private static createSlurs(
    ...
    for (const slur of musicXML.notation.getSlurs()) {
      ...
      const hammerOns = musicXML.notation
        .getTechnicals()
        .flatMap((t) => t.getHammerOns())
      const pullOffs = musicXML.notation
        .getTechnicals()
        .flatMap((t) => t.getPullOffs())
      if (slides.length > 0) {
  ...
  parse(voiceEntryCtx: VoiceEntryContext): string {
    if (this.phase === 'start') {
      return voiceEntryCtx.beginCurve(
        this.curveNumber,
        this.placement,
        this.opening,
        this.articulation,
