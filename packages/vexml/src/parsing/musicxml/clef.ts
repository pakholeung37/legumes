...
export class Clef {
  ...
  static create(
    ...
    log: Logger,
    partId: string,
    musicXML: { clef: musicxml.Clef },
  ) {
    const clefSign = conversions.fromClefPropertiesToClefSign(
  ...
  isEqual(clef: Clef): boolean {
    return (
      this.partId === clef.partId &&
      this.staveNumber === clef.staveNumber &&
      this.isEquivalent(clef)
    )
  }
