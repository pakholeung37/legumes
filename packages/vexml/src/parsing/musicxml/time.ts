...
export class Time {
  ...
  static create(
    ...
    if (symbol === 'single-number') {
      return Time.singleNumber(
        config,
        log,
        partId,
        staveNumber,
        Time.combine(config, log, partId, staveNumber, times),
      )
    }
    if (times.length === 1) {
      return times[0]
  ...
  private static parse(
    ...
    const numerators = beatsPerMeasure
      .split('+')
      .map((b) => parseInt(b.trim(), 10))

    if (numerators.length > 1) {
      const fractions = numerators.map(
        (numerator) => new util.Fraction(numerator, denominator),
      )
