...
export class JumpGroup {
  ...
  static create(
    ...
    const leftBarlines = barlines.filter(
      (barline) =>
        barline.getRepeatDirection() === 'forward' ||
        barline.getLocation() === 'left',
    )
    const rightBarlines = barlines.filter(
      (barline) =>
        barline.getRepeatDirection() === 'backward' ||
        barline.getLocation() === 'right',
    )
  ...
  getStartBarlineStyle(): data.BarlineStyle | null {
    return this.jumps.some((jump) => jump.type === 'repeatstart')
      ? 'repeatstart'
      : null
  }

  getEndBarlineStyle(): data.BarlineStyle | null {
    return this.jumps.some(
      (jump) =>
        jump.type === 'repeatend' ||
        (jump.type === 'repeatending' && jump.times > 0),
    )
      ? 'repeatend'
      : null
  }
