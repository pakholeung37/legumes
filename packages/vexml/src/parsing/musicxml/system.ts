...
export class System {
  ...
  static create(
    ...
    const partIds = musicXML.scorePartwise
      .getParts()
      .map((part) => part.getId())
    ...
    const measureCount = util.max(
      musicXML.scorePartwise
        .getParts()
        .map((part) => part.getMeasures().length),
    )
    const measureLabels = System.getMeasureLabels(measureCount, musicXML)
    const measureEvents = new EventCalculator(config, log, {
      scorePartwise: musicXML.scorePartwise,
    }).calculate()

    const jumpGroups = System.getJumpGroups(config, log, measureCount, musicXML)

    const startBarlineStyles = System.getBarlineStyles(
      measureCount,
      'left',
      musicXML,
      jumpGroups,
    )
    const endBarlineStyles = System.getBarlineStyles(
      measureCount,
      'right',
      musicXML,
      jumpGroups,
  ...
  private static getBarlineStyles(
    ...
    const barlineStyles = new Array<data.BarlineStyle | null>(
      measureCount,
    ).fill(null)
      ...
      const barlineStyle =
        ...
        conversions.fromMusicXMLBarStyleToBarlineStyle(
          musicXML.scorePartwise
            ...
            .flatMap(
              (part) =>
                part.getMeasures().at(measureIndex)?.getBarlines() ?? [],
            )
  ...
  parse(scoreCtx: ScoreContext): data.System {
    ...
    for (const measure of this.measures) {
      const multiRestEvents = measure
        .getEvents()
        .filter((event) => event.type === 'multirest')
