...
export class Note {
  ...
  static create(
    ...
    const pitch = new Pitch(
      config,
      log,
      musicXML.note.getStep(),
      musicXML.note.getOctave(),
    )
    const head = conversions.fromNoteheadToNotehead(musicXML.note.getNotehead())

    let durationType = conversions.fromNoteTypeToDurationType(
      musicXML.note.getType(),
    )
    let dotCount = musicXML.note.getDotCount()
    if (!durationType) {
      ;[durationType, dotCount] =
        conversions.fromFractionToDurationType(duration)
    }

    const stem = conversions.fromStemToStemDirection(musicXML.note.getStem())
    const annotations = musicXML.note
      .getLyrics()
      .map((lyric) => Annotation.fromLyric(config, log, { lyric }))

    const code =
      conversions.fromAccidentalTypeToAccidentalCode(
        musicXML.note.getAccidentalType(),
      ) ??
    ...
    if (!musicXML.note.isGrace()) {
      for (const graceNote of musicXML.note.getGraceNotes()) {
        ...
        if (graceNote.isChordHead()) {
          const tail = graceNote.getChordTail().map((note) =>
            Note.create(config, log, measureBeat, util.Fraction.zero(), {
              note,
            }),
          )
    ...
    const articulations = Articulation.create(config, log, {
      note: musicXML.note,
    })
    ...
    const fretPositions = TabPosition.create(config, log, {
      note: musicXML.note,
    })
  ...
  parse(voiceCtx: VoiceContext): data.Note {
    const voiceEntryCtx = VoiceEntryContext.note(
      voiceCtx,
      this.pitch.getStep(),
      this.pitch.getOctave(),
    )

    const tupletIds = util.unique([
      ...this.tuplets
        .map((tuplet) => tuplet.parse(voiceEntryCtx))
        .filter((id) => id !== null),
    ...
    return {
      ...
      articulations: this.articulations.map((articulation) =>
        articulation.parse(),
      ),
      bends: this.bends.map((bend) => bend.parse()),
      tabPositions: this.tabPositions.map((fretPosition) =>
        fretPosition.parse(),
      ),
    }
  }
  ...
  private maybeParseAccidental(
    voiceEntryCtx: VoiceEntryContext,
  ...
  private parseGraceChord(
    voiceEntryCtx: VoiceEntryContext,
    graceChord: GraceChord,
  ): data.GraceChord {
    const notes = [
      graceChord.head,
      ...graceChord.tail,
    ].map<data.GraceChordNote>((note) => ({
      type: 'gracechordnote',
      pitch: note.pitch.parse(),
      head: note.head,
      accidental: note.maybeParseAccidental(voiceEntryCtx),
