...
export class Articulation {
  ...
  static create(
    ...
    function add(
      ...
      placement: data.ArticulationPlacement,
    ) {
      articulations.push(
        new Articulation(config, log, articulationType, placement),
      )
    }
    ...
    musicXML.note
      ...
      .forEach((technical) => {
        ...
        technical
          .getDoubleTongues()
          .forEach(() => add('double-tongue', 'above'))
        technical
          .getTripleTongues()
          .forEach(() => add('triple-tongue', 'above'))
        technical.getStopped().forEach(() => add('stopped', 'above'))
        technical
          .getSnapPizzicatos()
          .forEach(() => add('snap-pizzicato', 'above'))
    ...
    musicXML.note
      ...
      .forEach((articulation) => {
        articulation
          .getAccents()
          .forEach((a) => add('accent', a.placement ?? 'above'))
        articulation
          .getStrongAccents()
          .forEach((a) => add('strong-accent', a.placement ?? 'above'))
        articulation
          .getStaccatos()
          .forEach((a) => add('staccato', a.placement ?? 'above'))
        articulation
          .getTenutos()
          .forEach((a) => add('tenuto', a.placement ?? 'above'))
        articulation
          .getDetachedLegatos()
          .forEach((a) => add('detached-legato', a.placement ?? 'above'))
        articulation
          .getStaccatissimos()
          .forEach((a) => add('staccatissimo', a.placement ?? 'above'))
        articulation
          .getScoops()
          .forEach((a) => add('scoop', a.placement ?? 'above'))
        articulation
          .getDoits()
          .forEach((a) => add('doit', a.placement ?? 'above'))
        articulation
          .getFalloffs()
          .forEach((a) => add('falloff', a.placement ?? 'above'))
        articulation
          .getBreathMarks()
          .forEach((a) => add('breath-mark', a.placement ?? 'above'))
      })
    ...
    musicXML.note
      ...
      .forEach((ornament) => {
        ...
        ornament
          .getInvertedMordents()
          .forEach(() => add('inverted-mordent', 'above'))
      })
