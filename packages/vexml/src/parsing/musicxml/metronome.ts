...
export class Metronome {
  ...
  static create(
    ...
    const duration =
      conversions.fromNoteTypeToDurationType(musicXML.mark.left.unit) ??
      undefined
    ...
    switch (musicXML.mark.right.type) {
      case 'note':
        const duration2 =
          conversions.fromNoteTypeToDurationType(musicXML.mark.right.unit) ??
          undefined
      ...
      case 'bpm':
        ...
        return new Metronome(config, log, displayBpm, {
          parenthesis,
          duration,
          dots,
          displayBpm,
