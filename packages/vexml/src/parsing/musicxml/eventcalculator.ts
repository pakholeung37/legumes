...
export class EventCalculator {
  ...
  private processNote(
    ...
    if (note.isChordHead()) {
      this.events.push({
        ...
        duration,
        chord: Chord.create(this.config, this.log, this.measureBeat, duration, {
          note,
        }),
      })
    } else if (note.isRest()) {
      this.events.push({
        ...
        measureBeat: this.measureBeat,
        duration,
        rest: Rest.create(this.config, this.log, this.measureBeat, duration, {
          note,
        }),
      })
    } else {
      this.events.push({
        ...
        measureBeat: this.measureBeat,
        duration,
        note: Note.create(this.config, this.log, this.measureBeat, duration, {
          note,
        }),
      })
    }
  ...
  private processAttributes(
    ...
    this.quarterNoteDivisions =
      attributes.getQuarterNoteDivisions() ?? this.quarterNoteDivisions

    const staveCount =
      attributes.getStaveCount() ?? this.previousExplicitStaveCount[partId]
    ...
    const staveLineCounts = attributes
      .getStaveDetails()
      .map((staveDetails) =>
        StaveLineCount.create(this.config, this.log, partId, { staveDetails }),
      )
    ...
    const clefs = attributes
      .getClefs()
      .map((clef) => Clef.create(this.config, this.log, partId, { clef }))
    ...
    const times = attributes
      ...
      .flatMap((time) => {
        ...
        if (typeof staveNumber === 'number') {
          ...
          return Time.createMulti(this.config, this.log, partId, staveCount, {
            time,
          })
        }
      })
    ...
    const measureStyle = attributes
      .getMeasureStyles()
      .find((measureStyle) => measureStyle.getMultipleRestCount() > 0)
    if (measureStyle) {
      this.events.push({
        type: 'multirest',
        partId,
  ...
  private processDirection(
    ...
    const dynamicType = direction
      ...
      .at(0)
    if (dynamicType) {
      const staveNumber = this.resolveStaveNumber(
        partId,
        direction.getStaveNumber(),
      )
    ...
    const octaveShift = direction
      .getOctaveShifts()
      .map((octaveShift) =>
        OctaveShift.create(this.config, this.log, { octaveShift }),
      )
      .at(0)
    if (octaveShift) {
      const staveNumber = this.resolveStaveNumber(
  ...
  private resolveVoiceId(partId: string, voiceId: string | null): string {
    return (this.previousExplicitVoiceId[partId] =
      voiceId ?? this.previousExplicitVoiceId[partId])
  }
  ...
  private resolveStaveNumber(
    ...
    return (this.previousExplicitStaveNumber[partId] =
      staveNumber ?? this.previousExplicitStaveNumber[partId])
  }
}
