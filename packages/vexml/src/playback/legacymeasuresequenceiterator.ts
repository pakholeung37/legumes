 ...
 */
export class LegacyMeasureSequenceIterator<T extends Measure>
  implements Iterable<number>
{
  ...
  [Symbol.iterator](): Iterator<number> {
    ...
    function getState() {
      ...
      const measureRepeats = repeats.filter(
        (repeat) => repeat.to === measureIndex,
      )

      let measureRepeatIndex = measureRepeats.findIndex((repeat) =>
        activeRepeat?.matches(repeat),
      )
    ...
    const iterator: Iterator<number> = {
      next: () => {
        ...
        if (
          isMeasureRepeatActive &&
          activeRepeat.isFinished() &&
          nextMeasureRepeat
        ) {
          activeRepeats.pop()
...
class Repeat {
  ...
  static from(measures: Measure[]): Repeat[] {
    ...
    for (let measureIndex = 0; measureIndex < measures.length; measureIndex++) {
      ...
      const hasRepeatEnding = measure.jumps.some(
        (jump) => jump.type === 'repeatending',
      )
      ...
      for (const jump of measure.jumps) {
        ...
        if (jump.type === 'repeatending' && jump.times > 0) {
          ...
          while (
            i > startMeasureIndex &&
            measures[i].jumps.some((jump) => jump.type === 'repeatending')
          ) {
            excluding.push(i)
            i--
          }
  ...
  decrement() {
    if (this.times === 0) {
      throw new Error(
        'Cannot decrement a repeat that has already been exhausted.',
      )
    }
    this.times--
  }
