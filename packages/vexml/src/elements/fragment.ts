...
export class Fragment {
  ...
  static create(
    ...
    const parts = fragmentRender.partRenders.map((partRender) =>
      Part.create(config, log, document, partRender),
    )
  ...
  getBpm(): number {
    return (
      this.document.getFragment(this.fragmentRender.key).signature.metronome
        .playbackBpm || 1
    ) // disallow 0;
  }
  ...
  isNonMusicalGap(): boolean {
    return (
      this.document.getFragment(this.fragmentRender.key).kind === 'nonmusical'
    )
  }

  /** Returns the non-musical duration of the gap. */
  getNonMusicalDurationMs(): number {
    return this.document.getNonMusicalFragment(this.fragmentRender.key)
      .durationMs
  }
}
