...
export class Note {
  ...
  containsEquivalentPitch(otherNote: Note): boolean {
    ...
    return this.getPitches().some((pitch) =>
      otherNote
        .getPitches()
        .some((otherPitch) => this.isPitchEqivalent(pitch, otherPitch)),
    )
  }

  /** Returns whether the note is connected to another note via a curve (tie or slur). */
  sharesACurveWith(otherNote: Note): boolean {
    return this.noteRender.curveIds.some((curveId) =>
      otherNote.noteRender.curveIds.includes(curveId),
    )
  }

  /** Returns the measure beat that this note starts on. */
  getStartMeasureBeat(): Fraction {
    return Fraction.fromFractionLike(
      this.document.getVoiceEntry(this.noteRender.key).measureBeat,
    )
  }
  ...
  getBeatCount(): Fraction {
    return Fraction.fromFractionLike(
      this.document.getVoiceEntry(this.noteRender.key).duration,
    )
  }
  ...
  private isPitchEqivalent(a: Pitch, b: Pitch): boolean {
    return (
      a.step === b.step &&
      a.octave === b.octave &&
      a.accidentalCode === b.accidentalCode
    )
  }
}
