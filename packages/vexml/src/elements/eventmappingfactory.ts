...
export class EventMappingFactory {
  ...
  static create(
    ...
    return new EventMappingFactory(
      root,
      elementLocator,
      timestampLocator,
      topic,
    ...
    return {
      ...
      native: {
        ...
        mousemove: (event) => {
          if (
            isPending &&
            lastMouseDownPoint.distance(this.point(event)) >
              MOUSEDOWN_MOVEMENT_TOLERANCE
          ) {
      ...
      native: {
        mousemove: (event) => {
          ...
          if (
            lastResult &&
            lastResult.target &&
            lastResult.target !== result.target
          ) {
        ...
        touchend: (event) => {
          if (isPending) {
            const {
              ...
              target: closestTarget,
              timestampMs,
            } = this.locate(event.changedTouches[0])
            if (closestTarget) {
              this.topic.publish('click', {
                type: 'click',
                target: closestTarget,
