...
export class Measure {
  ...
  getPartCount(): number {
    return Math.max(
      0,
      ...this.fragments.map((fragment) => fragment.getPartCount()),
    )
  }
  ...
  includesAbsoluteMeasureIndex(absoluteMeasureIndex: number): boolean {
    ...
    if (multiMeasureCount > 1) {
      return new util.NumberRange(
        start,
        start + multiMeasureCount - 1,
      ).includes(absoluteMeasureIndex)
    } else {
  ...
  getBeatCount(): Fraction {
    ...
    if (!time) {
      this.log.warn(
        'could not find time signature for measure',
        this.measureRender.key,
      )
      return Fraction.zero()
    }
    ...
    const beatsPerMeasure = Fraction.sum(...components).multiply(
      new Fraction(4),
    )
