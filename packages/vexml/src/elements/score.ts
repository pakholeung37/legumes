...
export class Score {
  ...
  static create(
    ...
    const systems = scoreRender.systemRenders.map((systemRender) =>
      System.create(config, log, document, systemRender),
    )
  ...
  addCursor(opts?: {
    ...
    util.assert(
      ...
      'partIndex out of bounds',
    )

    const span = opts?.span ?? { fromPartIndex: 0, toPartIndex: partCount - 1 }
    util.assert(
      0 <= span.fromPartIndex && span.fromPartIndex < partCount,
      'fromPartIndex out of bounds',
    )
    util.assert(
      0 <= span.toPartIndex && span.toPartIndex < partCount,
      'toPartIndex out of bounds',
    )
    util.assert(
      span.fromPartIndex <= span.toPartIndex,
      'fromPartIndex must be less than or equal to toPartIndex',
    )

    const timeline = this.getTimelines().find(
      (timeline) => timeline.getPartIndex() === partIndex,
    )
    util.assertDefined(timeline)

    const elementDescriber = playback.ElementDescriber.create(this, {
      partIndex,
    })
    const frames = playback.DefaultCursorFrame.create(
      this.log,
      this,
      timeline,
      span,
    ...
    const cursor = playback.Cursor.create(
      path,
  ...
  getDurationMs(): number {
    return Math.max(
      0,
      ...this.getTimelines().map((timeline) => timeline.getDuration().ms),
    )
  }
  ...
  addEventListener<N extends keyof EventMap>(
    type: N,
    listener: EventListener<EventMap[N]>,
  ): number {
  ...
  render(): void {
    ...
    const partLabelGroupRenders = fragmentRenders.flatMap(
      (m) => m.partLabelGroupRender ?? [],
    )
    const partLabelRenders = partLabelGroupRenders.flatMap(
      (p) => p.partLabelRenders,
    )
    ...
    if (config.DEBUG_DRAW_SYSTEM_RECTS) {
      systemRenders.forEach((s) => {
        new rendering.DebugRect(config, log, `s${s.key.systemIndex}`, s.rect)
          .setContext(ctx)
          .draw()
      })
    }
    ...
    if (config.DEBUG_DRAW_MEASURE_RECTS) {
      measureRenders.forEach((m) => {
        new rendering.DebugRect(config, log, `m${m.key.measureIndex}`, m.rect)
          .setContext(ctx)
          .draw()
      })
    }
    ...
    if (config.DEBUG_DRAW_FRAGMENT_RECTS) {
      fragmentRenders.forEach((f) => {
        new rendering.DebugRect(config, log, `f${f.key.fragmentIndex}`, f.rect)
          .setContext(ctx)
          .draw()
      })
    }
    ...
    if (config.DEBUG_DRAW_PART_RECTS) {
      partRenders.forEach((p) => {
        new rendering.DebugRect(config, log, `p${p.key.partIndex}`, p.rect)
          .setContext(ctx)
          .draw()
      })
    }
    ...
    if (config.DEBUG_DRAW_STAVE_RECTS) {
      staveRenders.forEach((s) => {
        new rendering.DebugRect(config, log, `s${s.key.staveIndex}`, s.rect)
          .setContext(ctx)
          .draw()
      })
    }
    ...
    if (config.DEBUG_DRAW_VOICE_RECTS) {
      voiceRenders.forEach((v) => {
        new rendering.DebugRect(config, log, `v${v.key.voiceIndex}`, v.rect)
          .setContext(ctx)
          .draw()
      })
    }
