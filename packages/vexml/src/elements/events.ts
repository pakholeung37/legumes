...
export class Events {
  ...
  static create(
    ...
    const bridge = new events.NativeBridge<keyof EventMap>(
      root,
      mappings,
      nativeEventTopic,
      {
  ...
  addEventListener<N extends keyof EventMap>(
    ...
    if (
      !this.vexmlEventTopic.hasSubscribers(type) &&
      !this.bridge.isActivated(type)
    ) {
  ...
  removeEventListener(...ids: number[]): void {
    for (const id of ids) {
      ...
      if (
        !this.vexmlEventTopic.hasSubscribers(subscription.name) &&
        this.bridge.isActivated(subscription.name)
      ) {
