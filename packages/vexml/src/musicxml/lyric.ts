...
export class Lyric {
  ...
  private createSyllabic(
    syllabic: NamedElement<'syllabic'>,
  ): Extract<LyricComponent, { type: 'syllabic' }> {
  ...
  ): Extract<LyricComponent, { type: 'text' }> {
    const value = text.content().withDefault('').str()
    return { type: 'text', value }
  }

  private createElision(
    elision: NamedElement<'elision'>,
  ): Extract<LyricComponent, { type: 'elision' }> {
