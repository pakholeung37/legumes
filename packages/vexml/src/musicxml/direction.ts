...
export class Direction {
  ...
  getTypes(): DirectionType[] {
    return this.element
      .all('direction-type')
      .map((node) => new DirectionType(node))
  }
  ...
  getSegnos(): Segno[] {
    return this.getTypes()
      ...
      .filter(
        (content): content is SegnoDirectionTypeContent =>
          content.type === 'segno',
      )
  ...
  getCodas(): Coda[] {
    return this.getTypes()
      .map((type) => type.getContent())
      .filter(
        (content): content is CodaDirectionTypeContent =>
          content.type === 'coda',
      )
  ...
  getOctaveShifts(): OctaveShift[] {
    return this.getTypes()
      .map((type) => type.getContent())
      .filter(
        (content): content is OctaveShiftDirectionTypeContent =>
          content.type === 'octaveshift',
      )
  ...
  getDynamics(): Dynamics[] {
    return this.getTypes()
      .map((type) => type.getContent())
      .filter(
        (content): content is DynamicsDirectionTypeContent =>
  ...
  getMetronome(): Metronome | null {
    return util.first(
      this.getTypes()
        .map((type) => type.getContent())
        .filter(
          (content): content is MetronomeDirectionTypeContent =>
            content.type === 'metronome',
        )
  ...
  getTokens(): Array<Words | Symbolic> {
    return this.getTypes()
      .map((type) => type.getContent())
      .filter(
        (content): content is TokensDirectionTypeContent =>
          content.type === 'tokens',
      )
  ...
  getWedges(): Wedge[] {
    return this.getTypes()
      .map((type) => type.getContent())
      .filter(
        (content): content is WedgeDirectionTypeContent =>
          content.type === 'wedge',
      )
  ...
  getPedals(): Pedal[] {
    return this.getTypes()
      .map((type) => type.getContent())
      .filter(
        (content): content is PedalDirectionTypeContent =>
          content.type === 'pedal',
      )
