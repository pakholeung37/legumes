...
export class Defaults {
  ...
  getStaveLayouts(): StaveLayout[] {
    return this.element.all('staff-layout').map((element) => ({
      ...
      staveDistance:
        element.first('staff-distance')?.content().withDefault(0).int() ?? null,
    }))
  }
  ...
  getSystemLayout(): SystemLayout {
    const leftMargin =
      this.element.first('left-margin')?.content().withDefault(0) ?? null
    const rightMargin =
      this.element.first('right-margin')?.content().withDefault(0) ?? null
    const topSystemDistance =
      this.element.first('top-system-distance')?.content().withDefault(0) ??
      null
    const systemDistance =
      this.element.first('system-distance')?.content().withDefault(0) ?? null
    return {
