...
export class Fraction {
  constructor(
    ...
    if (!Number.isInteger(this.numerator)) {
      throw new Error(
        'numerator must be an integer, try Fraction.fromDecimal instead',
      )
    }
    if (!Number.isInteger(this.denominator)) {
      throw new Error(
        'denominator must be an integer, try Fraction.fromDecimal instead',
      )
    }
  }
  ...
  static fromMixed(mixed: MixedFraction): Fraction {
    ...
    return new Fraction(mixed.whole * denominator, denominator).add(
      mixed.remainder,
    )
  }
