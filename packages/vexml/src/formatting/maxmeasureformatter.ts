...
export class MaxMeasureFormatter implements Formatter {
  ...
  constructor(
    private maxMeasuresPerSystemCount: number,
    opts?: MaxMeasureFormatterOptions,
  ) {
    this.config = { ...DEFAULT_CONFIG, ...opts?.config }
    this.log = opts?.logger ?? new NoopLogger()

    util.assert(
      maxMeasuresPerSystemCount > 0,
      'maxMeasuresPerSystemCount must be greater than 0',
    )
  }
  ...
  format(document: data.Document): data.Document {
    ...
    for (
      ...
      index += this.maxMeasuresPerSystemCount
    ) {
