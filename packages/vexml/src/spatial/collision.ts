...
class CollisionSubject {
  ...
  collidingWith(shape: Shape): boolean {
    ...
    throw new Error(
      `unsupported collision between ${shape1.constructor.name} and ${shape2.constructor.name}`,
    )
  }
  ...
  private isRectSurroundingCircle(rect: Rect, circle: Circle) {
    ...
    if (
      distanceX <= halfRectWidth - circle.r &&
      distanceY <= halfRectHeight - circle.r
    ) {
      return true
    }
    const cornerDistanceSq =
      (distanceX - halfRectWidth) ** 2 + (distanceY - halfRectHeight) ** 2
