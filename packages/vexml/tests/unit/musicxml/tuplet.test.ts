...
describe(Tuplet, () => {
  describe('getType', () => {
    it.each(START_STOP.values)(
      `returns the type of the tuplet: '%s'`,
      (type) => {
  ...
  describe('getPlacement', () => {
    it.each(ABOVE_BELOW.values)(
      `returns the placement of the tuplet: '%s'`,
      (placement) => {
  ...
  describe('getShowNumber', () => {
    it.each(SHOW_TUPLET.values)(
      'returns the show-number attribute',
      (showNumber) => {
