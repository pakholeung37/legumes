...
describe(Defaults, () => {
  describe('getStaveLayouts', () => {
    it('returns stave layouts', () => {
      const node = xml.defaults({
        staffLayouts: [
          xml.staffLayout({
            number: 1,
            staffDistance: xml.staffDistance({ value: '42' }),
          }),
          xml.staffLayout({
            number: 2,
            staffDistance: xml.staffDistance({ value: '43' }),
          }),
        ],
      })
    ...
    it('defaults number to 1 when invalid', () => {
      ...
      const defaults = new Defaults(node)
      expect(defaults.getStaveLayouts()).toStrictEqual([
        { staveNumber: 1, staveDistance: null },
      ])
    })
    ...
    it('defaults staff distance to 0 when invalid', () => {
      const node = xml.defaults({
        staffLayouts: [
          xml.staffLayout({
            number: 1,
            staffDistance: xml.staffDistance({ value: 'NaN' }),
          }),
        ],
      })
      const defaults = new Defaults(node)
      expect(defaults.getStaveLayouts()).toStrictEqual([
        { staveNumber: 1, staveDistance: 0 },
      ])
    })
  })
  ...
  describe('getSystemLayout', () => {
    ...
    it('defaults 0 for an invalid left margin', () => {
      const node = xml.defaults({
        systemLayout: xml.systemLayout({
          systemMargins: xml.systemMargins({
            leftMargin: xml.leftMargin({ tenths: NaN }),
          }),
        }),
      })
      const defaults = new Defaults(node)
    ...
    it('defaults 0 for an invalid right margin', () => {
      const node = xml.defaults({
        systemLayout: xml.systemLayout({
          systemMargins: xml.systemMargins({
            rightMargin: xml.rightMargin({ tenths: NaN }),
          }),
        }),
      })
      const defaults = new Defaults(node)
