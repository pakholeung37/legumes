...
describe(Measure, () => {
  ...
  describe('getAttributes', () => {
    it('returns the attributes elements', () => {
      ...
      expect(measure.getAttributes()).toStrictEqual([
  ...
  describe('getBarlines', () => {
    it('returns the barlines of the measure', () => {
      ...
      expect(measure.getBarlines()).toStrictEqual([
        new Barline(barline1),
        new Barline(barline2),
      ])
    })
  ...
  describe('getPrints', () => {
    it('returns the prints of the measure', () => {
      ...
      expect(measure.getPrints()).toStrictEqual([
        new Print(print1),
        new Print(print2),
      ])
    })
  ...
  describe('getEntries', () => {
    it('returns the notes, backups, and forwards of the measure', () => {
      ...
      const backup = xml.backup({
        duration: xml.duration({ positiveDivisions: 4 }),
      })
      const attributes2 = xml.attributes()
      const note2 = xml.note()
      const forward = xml.forward({
        duration: xml.duration({ positiveDivisions: 8 }),
      })
  ...
  describe('getEndingMeasure', () => {
    it('returns the ending measure for multi rest measures', () => {
      ...
      expect(new Measure(measure1).getEndingMeasure()).toStrictEqual(
        new Measure(measure3),
      )
    })
