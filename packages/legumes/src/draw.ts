...
let id_registry: Record<string, NoteRegister> = {}
...
function draw_slurs(score: Score): Element[] {
  ...
  for (let i = 0; i < score.slurs.length; i++) {
    ...
    function get_begin(reg: NoteRegister) {
...
function draw_tempo(tempo: TempoItf): Element[] {
  let result: Element[] = []
  let dx = 0
...
export function draw_score(score: Score): [Element[], number] {
  id_registry = {}
