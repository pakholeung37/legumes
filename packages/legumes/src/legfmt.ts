import { note_name_to_staff_pos, chord_and_beam_staff, short_id } from './utils'
import {
  CLEF,
  ACCIDENTAL,
...
let ARTICULATION_SYMBOL = Object.fromEntries(
    ...
    if (x == 'end') {
      ...
      if (curr_state('staff')) {
        ...
        if (state.includes('grace')) {
          begin +=
            staff.grace[begin0].staves[0].notes[
              staff.grace[begin0].staves[0].notes.length - 1
            ].duration
        } else {
        ...
        let sum =
          tup.members[tup.members.length - 1].begin -
          tup.begin +
          tup.members[tup.members.length - 1].duration
        let remain = tup.dur - sum
        for (let i = tup.members.length - 1; i >= 0; i--) {
          if (
            tup.members[i].begin == tup.members[tup.members.length - 1].begin
          ) {
      ...
      if (
        state[state.length - 2] == 'grace' &&
        staff.grace[begin0] &&
        staff.grace[begin0].staves[0].notes.length
      ) {
      ...
      if (x == '{') {
        while (
          score.instruments[score.instruments.length - 1] &&
          score.instruments[score.instruments.length - 1].connect_barlines
            .length <
            score.instruments[score.instruments.length - 1].names.length
        )
          score.instruments[score.instruments.length - 1].connect_barlines.push(
            false,
          )
...
function compile_staff(staff: StaffItf, force_stem_dir: number = 0) {
  ...
  function get_beat_length(time_sig: TimeSignature) {
    if (CONFIG.BEAM_POLICY == 0) {
      ...
      beat_length *=
        time_sig[0] % 2 || (time_sig[0] <= 2 && time_sig[1] > 2)
          ? time_sig[0]
          : time_sig[0] / 2
      return beat_length
    }
      ...
      for (let k = 0; k < staff.voices; k++) {
        ...
        for (let l = 0; l < staff.grace.length; l++) {
          ...
          let grace_item: [
            ...
            'grace',
            { begin: l - 0.001, duration: staff.grace[l].duration, items: [] },
            false,
          ]
            ...
            if (items[l][1].tuplet) {
              function count(): string[] {
                ...
                for (let m = l; m < items.length; m++) {
                  if (
                    !items[m][1].tuplet ||
                    items[m][1].tuplet.id != items[l][1].tuplet.id
                  ) {
                    break
                  }
                  ...
                  real =
                    items[m][1].begin - items[l][1].begin + items[m][1].duration
                }
              ...
              if (!items[l - 1] || prev_tup_id === null) {
                ...
                prev_tup_id !== null &&
                prev_tup_id != items[l][1].tuplet.id
              ) {
            ...
            if (items[l][0] == 'note') {
              ...
              do {
                ...
                if (e.articulation) {
                  s.push(
                    'a' +
                      (ARTICULATION_SYMBOL[e.articulation] ?? e.articulation),
                  )
                }
                if (e.cue) {
                  s.push(
