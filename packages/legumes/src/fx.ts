...
let scaled_cosine = function (i: number): number {
...
function disturb_polylines(
  ...
  for (let i = 0; i < elements.length; i++) {
    if (
      elements[i].type == 'composer' ||
      elements[i].type == 'tempo' ||
      elements[i].type == 'title' ||
      elements[i].type == 'subtitle'
    ) {
...
export function export_sketch_svg(
  dr: Drawing,
  { noise_mul = 1 }: { noise_mul?: number } = {},
) {
