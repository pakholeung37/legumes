...
let files = Array.from(
  new Set(fs.readdirSync('samples').filter((x) => x.endsWith('.txt'))),
)
files.sort()
files = Object.fromEntries(
  files.map((x) => [x, fs.readFileSync('samples/' + x).toString()]),
)
...
function main() {
  ...
  window.downloadBin = function (filename, content) {
    var bl = new Blob([new Uint8Array(content)], {
  ...
  menu.innerHTML = menu2html({
    ...
    Samples: (function () {
      ...
      for (let k in sampleFiles) {
        let s = `sampleFiles['${k}']`
        o[k] = eval(
          `()=>CM.setValue(${s})+document.getElementById('compile').click();`,
        )
      }
      return o
    })(),
    Compile: {
      ...
      '---1': null,
      'Thinner stroke': () => {
        document.getElementById('out').innerHTML = document
          .getElementById('out')
          .innerHTML.replace(/stroke-width="(.*?)"/g, (_, n) => {
    ...
    for (let i = 0; i < midi_file.tracks.length; i++) {
      ...
      for (let j = 0; j < midi_file.tracks[i].events.length; j++) {
        ...
        if (e.type == 'NOTE_ON') {
          ...
          synths[id].triggerAttack(
            name_midi_key[e.data.key],
            now + T * spd + (epsilon += d_epsilon),
          )
