...
const {
  ...
  score_to_midi,
  export_gif,
} = require('../dist/legumes')
{
  const file = Array.from(
    new Set(fs.readdirSync('tests/midi').filter((x) => x.endsWith('.mid'))),
  ).sort()[Number(process.argv[2] || 0)]

  // CONFIG.PAGE_WIDTH = 4000;

  let bytes_in = Array.from(
    new Uint8Array(fs.readFileSync('tests/midi/' + file)),
  )
...
{
  const file = Array.from(
    new Set(fs.readdirSync('tests/txt').filter((x) => x.endsWith('.txt'))),
  ).sort()[Number(process.argv[3] || 0)]
